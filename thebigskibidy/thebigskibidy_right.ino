...
void setup() {
  ...
  //Stay in system state WAITING for 5 seconds
    ...
    while (millis() <= startTime + Circle_time) {
        turnLeftSlight(MAX_SPEED, PARTIAL_SPEED);
    }
        ...
        switch (currentState) {
            case SEARCHING: {
                ...
                if (distanceDetected <= ROBOT_RANGE && distanceDetected != 0) {
                    ...
                    stationaryTurnLeft(SP_SPEED); // change
                    i++;

                } else if (i < 72) {
                    stationaryTurnRight(SP_SPEED);
                    i++;
                } else {
                    i = 0;
                }
...
void turnLeftSlight(int speed, int halfSpeed)
{
    Serial.println("Moving right");
    analogWrite(LEFT_SPEED, speed);
    analogWrite(RIGHT_SPEED, halfSpeed);
